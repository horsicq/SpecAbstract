...
NFD_LX::LXINFO_STRUCT NFD_LX::getInfo(QIODevice *pDevice, XScanEngine::SCANID parentId, XScanEngine::SCAN_OPTIONS *pOptions, qint64 nOffset,
		...
		// Microsoft-specific handling (moved from SpecAbstract::LX_handle_Microsoft)
		{
			...
			for (qint32 i = 0; (i < nRichSignaturesCount) && (XBinary::isPdStructNotCanceled(pPdStruct)); i++) {
				...
				listRichDescriptions.append(NFD_MSDOS::MSDOS_richScan(rr.nId, rr.nVersion, rr.nCount,
																	 ...
																	 &(result.basic_info), DETECTTYPE_RICH, pPdStruct));
			}
